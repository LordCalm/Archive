...
void node::OK()
{
	...
	{
		MessageBox::Show("Данные были удалены или изменены", "Ошибка");
	}
	#endif
	return;
}
...
void list::ListOK()
{
	if (_head != NULL && _tail != NULL)
		return;
	MessageBox::Show("Ошибка списка:\nсписок не найден", "Ошибка");
}
void list::PrintList()
{
	link cur = _head;
...
void list::ArrayToList(type *Array, int size)
{
	...
	{
		MessageBox::Show("Список не содержит элементов", "Ошибка");
	}
}
