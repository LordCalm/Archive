...
void hash_table::Register(char* data)
{
	...
	if(hashed[x]->_size == 0 || hashed[x]->FindString(data) == 0) 
        hashed[x]->InsertAfter(1, data);
}
char* hash_table::FindInfo(char* data)
{
    int x = hash_f(data) % table_size;
    if (hashed[x]->_size != 0)
    {
        return hashed[x]->FindSubString(data);
    }
    char* massage = new char[strlen("Данные о пациенте отсутствуют") + 1];
    strcpy(massage, "Данные о пациенте отсутствуют");
    return massage;
}
...
void hash_table::SaveToFile()
{
    ...
    out.close();
    MessageBox::Show("Данные были успешно сохранены", "Сохранено");
}
...
void hash_table::LoadFromFile()
{
    ...
    if (!in) // проверка - существует ли такой файл
        MessageBox::Show("Открываемый файл не существует", "Ошибка");
    else
    {
